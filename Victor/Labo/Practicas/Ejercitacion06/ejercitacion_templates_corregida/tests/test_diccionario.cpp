...
TEST(diccionario, dicc_string_bool) {
	...
	d.definir("queso", true);
	ASSERT_FALSE(d.def("zapallo"));
	ASSERT_EQ(d.obtener("queso"), true);
	d.definir("zapallo", false);

	ASSERT_TRUE(d.def("queso"));
	ASSERT_TRUE(d.def("zapallo"));
	ASSERT_FALSE(d.def("miga"));
	ASSERT_EQ(d.obtener("queso"), true);
	ASSERT_EQ(d.obtener("zapallo"), false);
	d.definir("miga", false);
	ASSERT_TRUE(d.def("queso"));
	ASSERT_TRUE(d.def("zapallo"));
	ASSERT_TRUE(d.def("miga"));
	ASSERT_EQ(d.obtener("queso"), true);
	ASSERT_EQ(d.obtener("miga"), false);
}
#endif
